...
/**
 * Reusable component for displaying subcategories in a grid layout
 * Used when viewing a parent category page
 */
export function SubcategoryGrid({ parentCategory, children }: SubcategoryGridProps) {
        ...
        <div className="min-h-screen bg-white py-4 md:py-8">
            <div className="w-full mx-auto px-3 sm:px-6 lg:px-8">
                <div className="mb-3 md:mb-5">
                    <h1 className="text-lg md:text-2xl font-bold text-gray-900">{parentCategory.name}</h1>
                    {parentCategory.description && (
                        <p className="text-sm md:text-base text-gray-600 mt-2">{parentCategory.description}</p>
                    )}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-3 md:gap-4 lg:gap-5 xl:gap-6 w-full">
                    {children.map((category, index) => {
                        ...
                        // Routing: we use slug-resolution via `/services/[categorySlug]`
                        // (no nested `/services/[parentSlug]/[childSlug]` route exists)
                        const categoryUrl = `/services/${category.slug}`;
